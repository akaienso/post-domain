...
create or adoption, checked by `BoundResource` before any read or mutation — and `DriverFactory::for_mapping()` has exactly one production caller, which a structural test enforces). A
...
| 08 | Every ambiguous outcome test resolves by a provider read, through the driver the lease was bound to and never one chosen from current configuration; a create or adoption promotes its environment into `ssl_provider_environment`, and a bound resource whose environment has drifted receives zero reads and zero mutations, refused before any lease is acquired; a conclusive recovery leaves no recovery schedule behind; only a lost CAS is reported as fencing, and an unconfirmed commit claims nothing rather than re-reading its own connection; every precondition failure proves zero mutating provider calls; a failed finalization writes nothing, deletes nothing, logs nothing, and returns `FENCED`; reconciliation counts no zero-row update; force-local-delete cannot overwrite a lease |
